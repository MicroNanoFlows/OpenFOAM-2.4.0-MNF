...
{  
    ...
    //                 }
                ...
                if(measureErrors_)
                {
                    if(dsmcRhoN_[cell] > VSMALL && Ma_[cell] > VSMALL && gamma > VSMALL && particleConstantVolumeSpecificHeat[cell] > VSMALL)
                    {
                    ...
                    if(!isA<emptyPolyPatch>(patch))
                    {
                        if(!isA<cyclicPolyPatch>(patch))
                        {
                            ...
                            {       
                                ...
                                dsmcRhoN_.boundaryField()[j][k] = dsmcRhoN_[boundaryCells_[j][k]];
...
//             }
            ...
            forAll(rhoNBF_, j)
            {
                ...
                {                     
                    forAll(rhoN_.boundaryField()[j], k)
                    {
                        ...
                        {
                            UMean_.boundaryField()[j][k] = momentumBF_[j][k]*cloud_.nParticle()/(rhoM_.boundaryField()[j][k]*nAvTimeSteps);
                        }
                        else
                        {
                        ...
                        if(rhoNMean > VSMALL)
                        {
                            translationalT_.boundaryField()[j][k] = 2.0/(3.0*physicoChemical::k.value()*rhoNMean)
                        ...
                        {
                            rotationalT_.boundaryField()[j][k] = (2.0/physicoChemical::k.value())*(rotationalEBF_[j][k]/rotationalDofBF_[j][k]);
                        }
                        else
                        {
    ...
    //                     }
                        ...
                        if(speedOfSound > VSMALL)
                        {
                            Ma_.boundaryField()[j][k] = mag(UMean_.boundaryField()[j][k])/speedOfSound;
                        }
                        else
                        {
        ...
        if(time_.resetFieldsAtOutput())
        {
            ...
            forAll(vibrationalETotal_, iD)
            {
                ...
                {
                    vibrationalETotal_[iD][cell] = 0.0;
