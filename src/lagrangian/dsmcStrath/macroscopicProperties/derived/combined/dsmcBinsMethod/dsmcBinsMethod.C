        ...
        forAll(molsInCell, mIC)
        {
            ...
            if(n != -1)
            {
                if(iD != -1)
                {
                    ...
                    eu_[n] += cloud_.nParticle()*( p->ERot() + EVib )*(p->U().x());
                    ev_[n] += cloud_.nParticle()*( p->ERot() + EVib )*(p->U().y());
                    ew_[n] += cloud_.nParticle()*( p->ERot() + EVib )*(p->U().z());
                    e_[n] += cloud_.nParticle()*( p->ERot() + EVib );
