...
#include "hogmanayControlAllExits.H"
...
defineTypeNameAndDebug(hogmanayControlAllExits, 0);

addToRunTimeSelectionTable(agentController, hogmanayControlAllExits, dictionary);


void hogmanayControlAllExits::setBoundBox
(
...
hogmanayControlAllExits::hogmanayControlAllExits
(
    Time& t,
    agentCloud& cloud,
...
//     agentId_()
{
    ...
    
    {
        ...
        topRightAgentIds_ = ids.agentIds();
    }    
    
    {
        topLeftAgentIds_.clear();
        ...
        topLeftAgentIds_ = ids.agentIds();
    }   
    
    {
        bottomRightAgentIds_.clear();
        ...
        bottomRightAgentIds_ = ids.agentIds();
    }    
    
    {
        bottomLeftAgentIds_.clear();
        ...
        bottomLeftAgentIds_ = ids.agentIds();
    }    
...
void hogmanayControlAllExits::initialConfiguration()
{}

void hogmanayControlAllExits::controlBeforeVelocityI()
{}

void hogmanayControlAllExits::controlBeforeMove()
{}

void hogmanayControlAllExits::controlBeforeForces()
{

}

void hogmanayControlAllExits::controlDuringForces
(
    agent* molI,
    agent* molJ
)
...
void hogmanayControlAllExits::controlAfterForces()
{
        ...
        for (p = cloud_.begin(); p != cloud_.end(); ++p)
        {
            if(p().eventTracker() == 0)
            {
                if(findIndex(topAgentIds_, p().id()) != -1)
                {
                    ...
                    {
                        if(p().position().y() < YMax_)
                        {
                            desDir = vector(0.0, (boxTop_.midpoint() - p().position()).y(), 0.0);
                        }
                        else if(p().position().y() > YMax_)
                        {
                            desDir = vector((boxTop_.midpoint() - p().position()).x(), 0.0, 0.0);   
                        }
...
/*                    vector R = vector(cloud_.rndGen().scalar01(), cloud_.rndGen().scalar01(), 0.0);
            ...
            if(p().eventTracker() == 2)            
            {
                ...
                if(findIndex(bottomAgentIds_, p().id()) != -1)
                {
                    ...
                    if(findIndex(bottomRightAgentIds_, p().id()) != -1)
                    {
                        integer = 1.0;
                    }
                    if(findIndex(bottomLeftAgentIds_, p().id()) != -1)
                    {
                        integer = -1.0;
                    }                                   
                ...
                if(findIndex(topAgentIds_, p().id()) != -1)
                {
                    ...
                    else if(p().position().y() < Y_ && p().position().x() < XRight_ && p().position().x() > XLeft_)
                    {
                        desDir = vector(0.0, (boxTop_.midpoint() - p().position()).y(), 0.0);
                    }
                    else if(p().position().y() < Y_ && p().position().x() > XRight_)
                    {
                        desDir = vector((boxBottom_.midpoint() - p().position()).x(), 0.0, 0.0);
                    }
                    else if(p().position().y() < Y_ && p().position().x() < XLeft_)
                    {
                        desDir = vector((boxBottom_.midpoint() - p().position()).x(), 0.0, 0.0);
                    }
                    else if (p().position().y() > YMax_)                                           
                    {    
                        ...
                        if(findIndex(topRightAgentIds_, p().id()) != -1)
                        {
                            integer = 1.0;
                        }
                        
                        if(findIndex(topLeftAgentIds_, p().id()) != -1)
                        {
                            integer = -1.0;
                        }                
...
void hogmanayControlAllExits::initialiseBorders()
{
...
void hogmanayControlAllExits::controlAfterVelocityII()
{}

void hogmanayControlAllExits::calculateProperties()
{}

void hogmanayControlAllExits::output
(
...
scalar hogmanayControlAllExits::gaussianDistribution
(
    const scalar& mean,
    const scalar& range
