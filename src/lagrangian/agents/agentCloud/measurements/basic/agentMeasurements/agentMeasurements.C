...
agentMeasurements::agentMeasurements
(
    Time& t,
    const polyMesh& mesh
)
...
:
    ...
    fieldList_(measurementsDict_.lookup("measurements")),
    ...
    measurementsDuringForceCompSite_()
{
    ...
    if(fields_.size() > 0 )
    {
        fileName fieldPath(time_.path()/"measurements");
        ...
        forAll(fields_, f)
        {
            ...
            fields_[f] = autoPtr<agentMeasurement>
            (
                agentMeasurement::New(time_, mesh, cloud, fieldIDict)
            );
...
void agentMeasurements::createFields()
{
...
void agentMeasurements::calculateFields()
{
...
void agentMeasurements::measurementsDuringForceComputation
(
    agent* molI,
    agent* molJ
)
...
void agentMeasurements::measurementsDuringForceComputationSite
(
    agent* molI,
    agent* molJ,
    label sI,
...
void agentMeasurements::writeFields()
{
