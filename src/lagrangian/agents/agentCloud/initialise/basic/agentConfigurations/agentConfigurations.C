...
:
    ...
    configurationList_(initialiseDict_.lookup("agentConfigurations")),
    ids_(configurationList_.size()),
    configurations_(configurationList_.size())
{

    Info << nl << "Creating agentConfigurations: " << nl << endl;
    ...
    if(configurations_.size() > 0 )
    {
        forAll(configurations_, c)
        {
            ...
            configurations_[c] = autoPtr<agentConfiguration>
            (
                agentConfiguration::New(cloud, configurationIDict)
            );
...
void agentConfigurations::setInitialConfig()
{
    forAll(configurations_, c)
    {
