...
defineTypeNameAndDebug(exactCoordinates, 0);

addToRunTimeSelectionTable(agentConfiguration, exactCoordinates, dictionary);
...
void exactCoordinates::setInitialConfiguration()
{
...
//     const vector bulkVelocity(propsDict_.lookup("bulkVelocity"));
    ...
    if(id == -1)
    {
        FatalErrorIn("exactCoordinates::setInitialConfiguration()")
...
//     label N(readLabel(propsDict_.lookup("N")));
    ...
    vectorList positions = vectorList(propsDict_.lookup("positions"));
...
void exactCoordinates::setBoundBox
(
