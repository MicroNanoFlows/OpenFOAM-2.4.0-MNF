...
defineTypeNameAndDebug(groupBoxInitialise, 0);

addToRunTimeSelectionTable(agentConfiguration, groupBoxInitialise, dictionary);
...
groupBoxInitialise::groupBoxInitialise
(
    agentCloud& molCloud,
...
void groupBoxInitialise::setInitialConfiguration()
{
...
//         d.add(mag(v));
        ...
        if(cell != -1)
        {
            ...
            agent* agentI = cloud_.last();

            label tI = agentI->trackingNumber();
            
            trackingNumbers.append(tI);
...
void groupBoxInitialise::setBoundBox
(
