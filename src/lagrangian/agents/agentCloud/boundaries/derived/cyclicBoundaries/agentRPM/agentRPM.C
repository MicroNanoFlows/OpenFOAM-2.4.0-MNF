...
defineTypeNameAndDebug(agentRPM, 0);

addToRunTimeSelectionTable(agentCyclicBoundary, agentRPM, dictionary);
...
:
    agentCyclicBoundary(t, mesh, cloud, dict),
...
void agentRPM::calculateProperties()
{
    scalar nTot = nReflections_+nPasses_;
    
    if(nTot > 0)
    {
        scalar pEst = nReflections_/nTot;
        
        Info << "no. reflections = " << nReflections_
            << ", no. passes = " << nPasses_
            << ", tot = " << nTot
            << " ---- est of p = " << pEst 
            << endl;
...
void agentRPM::initialConfiguration()
{}


void agentRPM::control
(
    agent& p,
...
//          << endl;
    ...
    label& faceI_=p.face();
    label& cellI_=p.cell();
    label& tetFaceI_=p.tetFace();
    label& tetPtI_=p.tetPt();
    vector& position_=p.position();
...
//     Info<< "neighbour patch = " << cpp.neighbPatchName() 
//         << ", neighbour patch id = " << cpp.neighbPatchID()
//         << endl;
    
    // find probability of reflection 
    
    scalar prob = cloud_.rndGen().scalar01();
...
//     Info << "p = " << prob << endl;
    ...
    {
//         Info << " reflecting " << endl;
        reflect = true;
    }
    else if (prob < p_) // i've double checked this and this is correct 
    {
//         Info << " reflecting " << endl;
        reflect = true;
    }
...
// void agentRPM::correctAfterParallelTransfer
// (
//     polyMolecule& mol,
//     const label& patchi,
