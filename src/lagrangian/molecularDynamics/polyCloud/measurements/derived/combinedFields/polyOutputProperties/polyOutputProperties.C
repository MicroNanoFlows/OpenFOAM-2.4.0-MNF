...
void polyOutputProperties::calculateField()
{
    ...
    label singleStepNMols = 0;
    label singleStepNMolsGhost = 0;
    ...
    if (Pstream::parRun())
    {
        ...
        reduce(singleStepNMolsGhost, sumOp<label>());
        reduce(singleStepDOFs, sumOp<label>());
    }

    if (singleStepNMols)
    {
        if(singleStepNMolsGhost == 0)
        {
            Info<< "Number of molecules in system = "
                << singleStepNMols << nl
                << "Overall number density = "
                << static_cast<scalar>(singleStepNMols)/meshVolume_ << nl
	    ...
	    label totalMols = singleStepNMols + singleStepNMolsGhost;
            ...
            Info<< "Number of molecules in system = "
                ...
                << static_cast<scalar>(totalMols)/meshVolume_ << nl
