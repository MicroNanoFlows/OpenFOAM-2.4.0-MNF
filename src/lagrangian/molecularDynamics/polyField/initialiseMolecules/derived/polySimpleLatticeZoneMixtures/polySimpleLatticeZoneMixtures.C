...
#include "polySimpleLatticeZoneMixtures.H"
...
defineTypeNameAndDebug(polySimpleLatticeZoneMixtures, 0);
addToRunTimeSelectionTable(polyConfiguration, polySimpleLatticeZoneMixtures, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

// Construct from components
polySimpleLatticeZoneMixtures::polySimpleLatticeZoneMixtures
(
...
void polySimpleLatticeZoneMixtures::setInitialConfiguration()
{
    ...
    if(zoneId == -1)
    {
        FatalErrorIn("polySimpleLatticeZoneMixtures::setInitialConfiguration()")
    ...
    forAll(molIdNames, m)
    {
        label molId = findIndex(idList, molIdNames[m]);

        if(molId == -1)
        {
            FatalErrorIn("polySimpleLatticeZoneMixtures::setInitialConfiguration()")
                << "Cannot find molecule id: " << molIdNames[m] << nl << "in idList."
                << exit(FatalError);
        }
    ...
    numberDensities_.setSize(molIds_.size(), 0.0);

    if (mdInitialiseDict_.found("numberDensities"))
    {
        const List<scalar> rho(mdInitialiseDict_.lookup("numberDensities"));

        if (rho.size() != molIds_.size())
        {
            FatalErrorIn("Foam::polyMoleculeCloud::initialiseMolecules")
                << "size of numberDensities must be equal to the molIds list"
                 << nl
                << abort(FatalError);
        }
    ...
    {
        FatalErrorIn("Foam::polyMoleculeCloud::initialiseMolecules")
            << "numberDensities not specified " << nl
            << abort(FatalError);
    }

    scalar numberDensity = sum(numberDensities_);

    // set probabilities

    p_.setSize(molIds_.size()+1, 0.0);

    p_[0] = 0.0;

    for (label i = 1; i < p_.size(); i++)
    {
        p_[i] = (numberDensities_[i-1]/numberDensity) + p_[i-1];
    }

    Info << "probabilities: " << p_ << endl;
    ...
    List<scalar> addMols(molIds_.size(), 0.0);
        ...
        for (label j = 0; j < nMolsY; j++)
        {
            ...
            for (label k = 0; k < nMolsZ; k++)
            {
                ...
                if(cell != -1)
                {
                    ...
                    if(p == 1)
                    {
                        molId = molIds_[molIds_.size()-1];
                        addMols[molIds_.size()-1] += 1.0;
                    }
    ...
    scalar totalAddedMols = sum(addMols);

    Info << "mol ids: " << molIds_ << endl;
    Info << "added molecules: " << addMols << endl;

    forAll(addMols, i)
    {
        addMols[i] /= totalAddedMols;
    }

    Info << "probabilities of added molecules: " << addMols << endl;
