...
#include "polyTemperatureBerendsenBins.H"
...
defineTypeNameAndDebug(polyTemperatureBerendsenBins, 0);
addToRunTimeSelectionTable(polyStateController, polyTemperatureBerendsenBins, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

// Construct from components
polyTemperatureBerendsenBins::polyTemperatureBerendsenBins
(
    Time& t,
    ...
    output_(false)
{
    ...
    if (propsDict_.found("componentControl"))
    {
        ...
        if(componentControl_)
        {
            ...
            if(!X_ && !Y_ && !Z_)
            {
                FatalErrorIn("polyTemperatureBerendsenBins::polyTemperatureBerendsenBins()")
...
void polyTemperatureBerendsenBins::initialConfiguration()
{
        ...
        forAll(molsInCell, mIC)
        {
            ...
            if(n != -1)
            {
                if((findIndex(molIds_, molI->id()) != -1) || measureFullTemperature_)
                {
                    ...
                    momentum[n] += constProp.mass()*molI->v();
                }
            }
        }
    }
...
void polyTemperatureBerendsenBins::calculateProperties()
{
    const labelList& cells = mesh_.cellZones()[regionId_];
            ...
            forAll(molsInCell, mIC)
            {
                ...
                if(n != -1)
                {
                    if((findIndex(molIds_, molI->id()) != -1) || measureFullTemperature_)
                    {
                        ...
                        mass_[n] += constProp.mass();
                        momentum_[n] += constProp.mass()*molI->v();
                    }
                }
            }
        }
...
void polyTemperatureBerendsenBins::controlMolsBeg()
{}

void polyTemperatureBerendsenBins::controlMols()
{
}

void polyTemperatureBerendsenBins::controlBeforeForces()
{}

void polyTemperatureBerendsenBins::controlMolsEnd()
{
    if(control_)
    {
        Info << "polyTemperatureBerendsenBins: measurement and control" << endl;
...
void polyTemperatureBerendsenBins::controlDuringForces
(
    polyMolecule* molI,
    polyMolecule* molJ
)
...
void polyTemperatureBerendsenBins::output
(
...
void polyTemperatureBerendsenBins::updateProperties(const dictionary& newDict)
{
