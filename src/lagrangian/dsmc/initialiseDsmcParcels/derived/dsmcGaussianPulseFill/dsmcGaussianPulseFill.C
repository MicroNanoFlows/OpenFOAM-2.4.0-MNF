...
#include "dsmcGaussianPulseFill.H"
...
defineTypeNameAndDebug(dsmcGaussianPulseFill, 0);

addToRunTimeSelectionTable(dsmcConfiguration, dsmcGaussianPulseFill, dictionary);
...
dsmcGaussianPulseFill::dsmcGaussianPulseFill
(
    dsmcCloud& cloud,
...
void dsmcGaussianPulseFill::setInitialConfiguration()
{
    ...
    gaussianAmplitude_ = readScalar(dsmcInitialiseDict_.lookup("gaussianAmplitude"));
    
    sigma_ = readScalar(dsmcInitialiseDict_.lookup("sigma"));
    
    startPoint_ = readScalar(dsmcInitialiseDict_.lookup("startPoint"));
    ...
    if(zoneId == -1)
    {
        FatalErrorIn("dsmcGaussianPulseFill::setInitialConfiguration()")
            ...
            forAll(cellTets, tetI)
            {
                ...
                forAll(molecules, i)
                {
                    ...
                    scalar numberDensity = numberDensities[i]
                        + gaussianAmplitude_*numberDensities[i]*exp(-sqr(cC.x() - startPoint_)/(2.0*sigma_*sigma_));
    ...
    forAll(zone, c)
    {
        ...
        cloud_.sigmaTcRMax().internalField()[cellI] = cP.sigmaT()*cloud_.maxwellianMostProbableSpeed
        (
            translationalTemperature,
            cP.mass()
        );
