...
defineTypeNameAndDebug(dsmcSinePulseFill, 0);

addToRunTimeSelectionTable(dsmcConfiguration, dsmcSinePulseFill, dictionary);
...
dsmcSinePulseFill::dsmcSinePulseFill
(
    dsmcCloud& cloud,
...
void dsmcSinePulseFill::setInitialConfiguration()
{
    ...
    amplitude_ = readScalar(dsmcInitialiseDict_.lookup("amplitude"));
    
    startPoint_ = readScalar(dsmcInitialiseDict_.lookup("startPoint"));
    
    waveLength_ = readScalar(dsmcInitialiseDict_.lookup("waveLength"));
    ...
    if(zoneId == -1)
    {
        FatalErrorIn("dsmcSinePulseFill::setInitialConfiguration()")
            ...
            forAll(cellTets, tetI)
            {
                ...
                forAll(molecules, i)
                {
                    ...
                    scalar numberDensity = numberDensities[i]
                        + amplitude_*numberDensities[i]*sin(2.0*3.1416*((cC.x() - startPoint_)/waveLength_));
