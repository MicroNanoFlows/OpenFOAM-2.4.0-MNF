...
// 				}	
// 			}
// 		Info << "Number of parcels in cell :" << parcels_[c] << endl;
// 	     }
//     }
    ...
    // 				Info << "Parcel Id = " << p->typeId() << ", typeId_ = " << typeId_ << endl;
...
void densityController::controlParcelsBeforeMove()
{
    if((control_) && time_.controlTime())
    {
        ...
        forAll(nMolsToControl_, c)
        {
            ...
            //Info << "nMolsToControl = " << nMolsToControl_[c] << endl;
        }
        ...
        forAll(residualSum_, c)
        {
            ...
            if(residualSum_[c] > 1) // insert residual parcel
            {
                insertParcelWithinDSMC(c);
                residualSum_[c] =- 1;
                //Info << "INSERTED RESIDUAL PARCEL" << endl;
            }
            else if(residualSum_[c] < -1) // delete residual parcel
            {
                deleteParcelFromDSMC(c);
                residualSum_[c] =+ 1;
                //Info << "DELETED RESIDUAL PARCEL" << endl;
            }
        //Info << "Residual = " << residualSum_[c] << endl;
        }
...
void densityController::insertParcelWithinDSMC(const label& c)
{
    ...
    //const vector& cellVelocity = velocities_[c];

    //Info << "INS BEFORE: nParcels :" << molsInCell.size() << " velocity: " << cellVelocity << endl;
