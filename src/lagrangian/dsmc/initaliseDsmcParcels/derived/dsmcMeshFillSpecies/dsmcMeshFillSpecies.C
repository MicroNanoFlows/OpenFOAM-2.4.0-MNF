...
defineTypeNameAndDebug(dsmcMeshFillSpecies, 0);

addToRunTimeSelectionTable(dsmcConfiguration, dsmcMeshFillSpecies, dictionary);
...
dsmcMeshFillSpecies::dsmcMeshFillSpecies
(
    dsmcCloud& cloud,
...
void dsmcMeshFillSpecies::setInitialConfiguration()
{
...
//     );
    ...
    (
        dsmcInitialiseDict_.subDict("vibrationalTemperatures")
    );
    ...
    Field<scalar> vibrationalTemperatures(molecules.size());
    ...
    forAll(molecules, i)
    {
        ...
        vibrationalTemperatures[i] = readScalar
        (
            vibrationalTemperaturesDict.lookup(molecules[i])
        );
    }
    ...
    cloud_.sigmaTcRMax().internalField() = cP.sigmaT()*cloud_.maxwellianMostProbableSpeed
    (
        translationalTemperature,
        cP.mass()
    );
