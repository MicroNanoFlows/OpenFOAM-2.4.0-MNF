...
#   include "createRandom.H"
    ...
    forAll(timeDirs, timeI)
    {
        ...
        fileName path(runTime.path()/runTime.timeName() +  + "/lagrangian" + "/polyMoleculeCloud_velocity.xmol");

	OFstream os(path);

	os  << molecules.nSites() << nl << "polyMoleculeCloud site positions in angstroms, velocities in angstrom/femtosecond" << nl;
        
        //molecules.writeXYZ(path);

	IDLList<polyMolecule>::iterator mol(molecules.begin());

        for (mol= molecules.begin(); mol != molecules.end(); ++mol)
	  {
// 	    const polyMolecule::constantProperties& cP = molecules.constProps(mol().id());

	    forAll(mol().sitePositions(), i)
	      {
		const point& sP = mol().sitePositions()[i];
		
		os  << cP.siteNames()[mol().id()][i]
		    << ' ' << sP.x()*rU.refLength()*1.0e10
		    << ' ' << sP.y()*rU.refLength()*1.0e10
		    << ' ' << sP.z()*rU.refLength()*1.0e10
			<< ' ' << mol().v().x()*rU.refVelocity()*1.0e-5
		    << ' ' << mol().v().y()*rU.refVelocity()*1.0e-5
		    << ' ' << mol().v().z()*rU.refVelocity()*1.0e-5
		    << nl;
	      }
	  }
