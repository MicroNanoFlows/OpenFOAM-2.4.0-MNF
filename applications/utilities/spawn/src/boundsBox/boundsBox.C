...
const Foam::scalar Foam::boundsBox::great(VGREAT);

const Foam::boundsBox Foam::boundsBox::greatBox
(
...
const Foam::boundsBox Foam::boundsBox::invertedBox
(
    point(VGREAT, VGREAT, VGREAT),
    point(-VGREAT, -VGREAT, -VGREAT)
);
...
void Foam::boundsBox::calculate(const UList<point>& points, const bool doReduce)
{
    if (points.empty())
    {
        min_ = point::zero;
...
Foam::boundsBox::boundsBox(const UList<point>& points, const bool doReduce)
:
    min_(point::zero),
    max_(point::zero)
{
...
Foam::boundsBox::boundsBox(const tmp<pointField>& points, const bool doReduce)
:
    min_(point::zero),
    max_(point::zero)
{
...
Foam::tmp<Foam::pointField> Foam::boundsBox::points() const
{
...
void Foam::boundsBox::inflate(const scalar s)
{
...
void Foam::boundsBox::expand(const scalar s)
{
    vector v = max_-min_;
...
void Foam::boundsBox::expandII(const scalar s)
{
...
void Foam::boundsBox::contractII(const scalar s)
{
//     Info << "(before) min_ " << min_ << endl;
//     Info << "(before) max_ " << max_ << endl;
    min_ += vector(1,0,0)*s;
...
bool Foam::boundsBox::contains(const UList<point>& points) const
{
    if (points.empty())
    {
        return true;
...
bool Foam::boundsBox::containsAny(const UList<point>& points) const
{
    if (points.empty())
    {
        return true;
...
bool Foam::boundsBox::containsAny
(
    const UList<point>& points,
    const labelUList& indices
) const
...
void Foam::boundsBox::resetBoundedBox
(
...
Foam::Ostream& Foam::operator<<(Ostream& os, const boundsBox& bb)
{
    ...
    os.check("Ostream& operator<<(Ostream&, const boundsBox&)");
    return os;
}


Foam::Istream& Foam::operator>>(Istream& is, boundsBox& bb)
{
    ...
    is.check("Istream& operator>>(Istream&, boundsBox&)");
    return is;
}
